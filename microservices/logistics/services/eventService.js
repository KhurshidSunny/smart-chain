...
const connectRabbitMQ = async () => {
  try {
    ...
    console.log(
      `Queue ${queue} bound to ${exchange} with routing key 'warehouse.order.packed'`
    );
    ...
    console.error('RabbitMQ Connection Error (Logistics):', error.message);
    channel = null;
    setTimeout(() => {
...
const subscribeToEvents = (eventHandlers) => {
  if (!channel) {
    console.warn('RabbitMQ not ready — skipped subscribe (Logistics)');
    return false;
  }
